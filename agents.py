...
class ArchitectAgent(Agent):
    ...
    async def plan(self, blackboard: Blackboard, memory: MemoryVault, knowledge: KnowledgeManager, project_context: str = ""):
        ...
        if hasattr(blackboard, "correction_plan") and blackboard.correction_plan: full_context += f"\nCORRECTION: {blackboard.correction_plan}"
        candidates = []
        for _ in range(3):
            ...
            score = await self.simulate_plan(p, full_context); candidates.append((p, score))
        candidates.sort(key=lambda x: x[1], reverse=True)
        blackboard.plan = candidates[0][0]; blackboard.status = "Executing"

class ExecutorAgent(Agent):
    async def act(self, blackboard: Blackboard):
        if not blackboard.plan or blackboard.current_step_index >= len(blackboard.plan): return {"skill": "error", "error": "End"}
    ...
    async def self_repair(self, error: str, blackboard: Blackboard):
        return {"skill": "command", "params": {"cmd": "echo 'Zenith Repair...'"}}

class AuditorAgent(Agent):
    async def verify(self, last_action, blackboard: Blackboard):
        # Consensus: High-stakes if it's a file write or github operation
        high_stakes = last_action.get("skill") in ["file", "github", "execute_code"]
        return await self.vision.verify_outcome(last_action=last_action, screenshot=blackboard.last_screenshot, ui_tree_summary=json.dumps(blackboard.last_ui_tree), high_stakes=high_stakes)
    ...
    async def generate_correction_plan(self, failed_result: dict, blackboard: Blackboard):
        ...
        blackboard.correction_plan = correction; return correction
