...
class OmniAgentGUI(ctk.CTk):
    def __init__(self, orchestrator, blackboard):
        ...
        self.title("OmniAgent OS: Singularity"); self.geometry("1400x950")
        ...
        self.header = ctk.CTkFrame(self, height=70, fg_color="#0b0c10", corner_radius=15, border_width=1, border_color="#1f2833")
        self.header.grid(row=0, column=0, sticky="ew", padx=15, pady=10)

        self.goal_entry = ctk.CTkEntry(self.header, placeholder_text="Command the Singularity...", width=500, fg_color="#000", border_color="#00ffa3", corner_radius=10)
        self.goal_entry.pack(side="left", padx=20, pady=15)

        self.run_btn = ctk.CTkButton(self.header, text="TRANSCEND", fg_color="#00ffa3", text_color="#000", font=("SF Pro Display", 13, "bold"), corner_radius=10, command=self.start_task)
        self.run_btn.pack(side="left", padx=5)

        self.telemetry = ctk.CTkLabel(self.header, text="RAM: 0% | CPU: 0% | Net: OK", text_color="#00ffa3", font=("SF Mono", 11))
        self.telemetry.pack(side="right", padx=25)

        # Command Center Tabview
        self.tabs = ctk.CTkTabview(self, fg_color="#050505", segmented_button_selected_color="#00ffa3", segmented_button_selected_hover_color="#00e5ff", corner_radius=15)
        self.tabs.grid(row=1, column=0, sticky="nsew", padx=15, pady=5)

        self.tab_core = self.tabs.add("Core Intelligence")
        self.tab_neuro = self.tabs.add("Neuro-Sync")
        self.tab_visual = self.tabs.add("Visual Echo")
        self.tab_lab = self.tabs.add("Skill Factory")
        self.tab_sovereign = self.tabs.add("Sovereign Control")
        ...
        # Activity Monologue
        self.monologue = ctk.CTkTextbox(self, height=130, fg_color="#000", text_color="#00ffa3", font=("SF Mono", 11), corner_radius=10, border_width=1, border_color="#1f2833")
