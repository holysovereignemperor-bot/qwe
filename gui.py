...
class OmniAgentGUI(ctk.CTk):
    def __init__(self, orchestrator, blackboard):
        ...
        self.title("OmniAgent OS: Prime Origin"); self.geometry("1450x950")
        ...
        # Origin Header
        self.header = ctk.CTkFrame(self, height=80, fg_color="#0a0a0a", corner_radius=20)
        self.header.grid(row=0, column=0, sticky="ew", padx=20, pady=15)

        self.goal_entry = ctk.CTkEntry(self.header, placeholder_text="Prime Origin Command...", width=500, fg_color="#000", border_color="#00ffa3", corner_radius=15)
        self.goal_entry.pack(side="left", padx=25, pady=20)

        self.run_btn = ctk.CTkButton(self.header, text="ORIGIN", fg_color="#00ffa3", text_color="#000", font=("SF Pro Display", 14, "bold"), corner_radius=15, command=self.start_task)
        self.run_btn.pack(side="left", padx=10)

        self.cmd_bar_btn = ctk.CTkButton(self.header, text="Command Bar", width=120, fg_color="#1a1a1a", corner_radius=15, command=self.toggle_command_bar)
        self.cmd_bar_btn.pack(side="left", padx=10)

        self.telemetry = ctk.CTkLabel(self.header, text="PRIME: ONLINE", text_color="#00ffa3", font=("SF Mono", 12))
        ...
        self.tab_core = self.tabs.add("Origin Core"); self.tab_visual = self.tabs.add("Visual Stream")
    ...
    def create_card(self, parent, title, col):
        ...
        status = ctk.CTkLabel(card, text="Awaiting origin...", text_color="#444", wraplength=250); status.pack(pady=15)
    ...
    def toggle_command_bar(self):
        ...
        if self.is_command_bar:
            ...
            self.header.configure(corner_radius=30); self.cmd_bar_btn.configure(text="Exit Bar")
        else:
            self.geometry("1450x950"); self.tabs.grid(); self.monologue.grid()
            self.header.configure(corner_radius=20); self.cmd_bar_btn.configure(text="Command Bar")
