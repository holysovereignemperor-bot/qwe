...
class OmniAgentGUI(ctk.CTk):
    def __init__(self, orchestrator, blackboard):
        ...
        self.title("OmniAgent OS: Apex Command"); self.geometry("1450x950")
        ctk.set_appearance_mode("dark"); self.configure(fg_color="#000")

        # Native Menu Bar
        self.menubar = MenuBarController(self.start_task, lambda: setattr(self.blackboard, 'is_running', False))
        ...
        self.goal_entry = ctk.CTkEntry(self.header, placeholder_text="Apex Input...", width=500, fg_color="#050505", border_color="#00ffa3")
        self.goal_entry.pack(side="left", padx=30, pady=20)

        self.run_btn = ctk.CTkButton(self.header, text="INITIATE", fg_color="#00ffa3", text_color="#000", font=("SF Pro Display", 14, "bold"), command=self.start_task)
        self.run_btn.pack(side="left", padx=10)

        self.telemetry = ctk.CTkLabel(self.header, text="APEX ONLINE", text_color="#00ffa3", font=("SF Mono", 12))
        self.telemetry.pack(side="right", padx=35)
        ...
        self.tab_apex = self.tabs.add("Apex Logic")
        self.tab_omnisearch = self.tabs.add("Omni-Search")
        self.tab_visual = self.tabs.add("Visual Stream")
        self.tab_docs = self.tabs.add("Doc Factory")

        # Apex Logic cards
        self.tab_apex.grid_columnconfigure((0,1,2), weight=1); self.tab_apex.grid_rowconfigure(0, weight=1)
        self.pm_card = self.create_card(self.tab_apex, "Architect", 0)
        self.exec_card = self.create_card(self.tab_apex, "Executor", 1)
        self.qa_card = self.create_card(self.tab_apex, "Auditor", 2)
        ...
        self.monologue.grid(row=2, column=0, sticky="ew", padx=20, pady=25)
    ...
    def update_loop(self):
        ...
        self.telemetry.configure(text=f"RAM: {ram}% | CPU: {cpu}% | APEX")
